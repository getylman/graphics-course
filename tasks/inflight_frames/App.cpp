...
const auto kShaderName = "inflight_frames";
  ...
  , useVsync{true}
{
  ...
  etna::create_program(kTextureName, {INFLIGHT_FRAMES_SHADERS_ROOT "toy.vert.spv", INFLIGHT_FRAMES_SHADERS_ROOT "texture.frag.spv"});
  etna::create_program(kShaderName, {INFLIGHT_FRAMES_SHADERS_ROOT "toy.vert.spv", INFLIGHT_FRAMES_SHADERS_ROOT "toy.frag.spv"});

  pipeline = etna::get_context().getPipelineManager().createGraphicsPipeline(kTextureName, etna::GraphicsPipeline::CreateInfo {.fragmentShaderOutput = {.colorAttachmentFormats = {vk::Format::eB8G8R8A8Srgb}}});
  graphicsPipeline = etna::get_context().getPipelineManager().createGraphicsPipeline(kShaderName, etna::GraphicsPipeline::CreateInfo{.fragmentShaderOutput = {.colorAttachmentFormats = {vk::Format::eB8G8R8A8Srgb}}});
...
void App::drawFrame()
{
  ...
  if (!isFileTextureInit) {
    ...
    grafImage = etna::create_image_from_bytes(info, etna::get_context().createOneShotCmdMgr()->start(), file);

    stbi_image_free(file);
    isFileTextureInit = true;
  }
  ...
  if (nextSwapchainImage)
  {
    ...
    ETNA_CHECK_VK_RESULT(currentCmdBuf.begin(vk::CommandBufferBeginInfo{}));
    {
      ...
      ETNA_PROFILE_GPU(currentCmdBuf, kFrameName);
      ...
      const float kTime = static_cast<int64_t>(std::chrono::duration_cast<std::chrono::milliseconds>(
      ...
      {
        ETNA_PROFILE_GPU(currentCmdBuf, "ProceduralTexture");

        etna::RenderTargetState state{currentCmdBuf, {{}, {resolution.x, resolution.y}},{{bufImage.get(), bufImage.getView({})}}, {}};

        currentCmdBuf.bindPipeline(vk::PipelineBindPoint::eGraphics, pipeline.getVkPipeline());

        currentCmdBuf.pushConstants(pipeline.getVkPipelineLayout(), vk::ShaderStageFlagBits::eFragment, 0, sizeof(params), &params);
...

      {
        ...
        std::memcpy(flameBuffer.buf[flameBuffer.counter].data(), &params, sizeof(Params));
