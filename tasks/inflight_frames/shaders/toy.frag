...
{
   vec3 q = m * p;
	return dSerpinskiy (q, vec2 (2, 0.5), 2.0 ) ;
}
...
vec3 get_texture_color( in vec3 p, in vec3 n, in mat3 m )
{
    vec3 q = m * p;
    vec3 new_n = m * n;
    vec3 w = abs(new_n);
    ...
    vec3 albedo = w.x * m * texture(iChannel0, q.yz).rgb +
                  w.y * m * texture(iChannel0, q.xz).rgb + 
                  w.z * m * texture(iChannel0, q.xy).rgb;
                  
    return albedo * 1.5;
    
}

vec4 cookTorrance ( in vec3 p, in vec3 n, in vec3 l, in vec3 v, in mat3 m )
{
    ...
    vec3  cur_clr = get_texture_color( p, n, m );
    vec3  base  = pow ( cur_clr, vec3 ( gamma ) );
    vec3  F0    = mix ( vec3(FDiel), cur_clr, metallness );
...
vec3 skybox_color(in vec3 direction) {
	...
	vec2 uv = gl_FragCoord.xy / iResolution.xy*zoom-zoom/2.0;
...
void main()
{
  ...
  vec4 color = vec4 ( skybox_color(dir), 1 );
